...
Такие библиотеки как Boost.Asio обычно используются для достижения большей эффективности. При этом нет необходимости ждать завершения функции, программа может выполнять в этот момент другие задачи. ...Таким образом, можно запустить несколько асинхронных операций, которые все выполнены одновременно - помните, что асинхронные операции, как правило, используются для доступа к ресурсам за пределами процесса. Поскольку эти ресурсы могут быть различные устройства, они могут работать независимо друг от друга и выполнять операции одновременно.

Масштабируемость характеризует способность программы эффективно извлечь выгоду из дополнительных ресурсов. С помощью Boost.Asio можно извлечь выгоду из возможности внешних устройств для выполнения операций одновременно. Если используются нити, несколько функций могут выполняться одновременно на доступных ядер процессора. Boost.Asio с потоками улучшает масштабируемость, поскольку ваша программа может воспользоваться внутренними и внешними устройствами, которые могут выполнять операции самостоятельно или в сотрудничестве друг с другом.
