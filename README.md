...
Вместо вызова блокирующей функции, которая будет возвращаться, когда будильник зазвонит, Boost.Asio позволяет начать асинхронную операцию. Чтобы это сделать, вызовите функцию ```async_wait()```, которая ожидает обработчик в качестве единственного параметра. Обработчик является функцией или функциональный объект, который вызывается, когда асинхронная операция завершается. В [примере 32.1](#example321) в качестве параметра передается лямбда - функция.
