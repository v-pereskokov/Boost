...
Вместо вызова функции блокировки, которая будет возвращать, когда будильник зазвонит, Boost.Asio позволяет начать асинхронную операцию. Чтобы сделать это, вызовите функцию async_wait член (), которая ожидает обработчик в качестве единственного параметра. Обработчик является функцией или объект функции, которая вызывается, когда асинхронная операция завершается. В примере 32.1, функция лямбда передается в качестве обработчика.
